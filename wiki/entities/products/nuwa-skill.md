---
title: "nuwa-skill"
type: entity
subtype: product
tags: [mind-model, ai-skill, thinking-distillation, claude-code, multi-agent, chinese]
created: 2026-04-15
updated: 2026-04-20
sources:
  - ../../raw/morning-briefs/2026-04-15.md
  - ../../raw/morning-briefs/2026-04-20.md
  - ../../../raw/articles/nuwa-skill-github.md
---

# nuwa-skill（女娲·Skill造人术）

> 蒸馏任何人的思维框架为可运行 Claude Code Skill——六路 Agent 并行调研、三重验证、Agentic 回答工作流，13K Stars

## 基本信息

- **GitHub**: alchaincyf/nuwa-skill
- **作者**: 花叔 (alchaincyf)，独立 AI 开发者
- **Stars**: 13,000+ (2026-04-20)，较早期的 11.3K 持续增长 | **Forks**: 1.9K+
- **License**: MIT
- **语言**: Python (88.9%), Shell (11.1%)
- **安装**: `npx skills add alchaincyf/nuwa-skill`
- **灵感来源**: colleague-skill (titanwings)

## 核心能力

将个人/专家的认知操作系统提炼为五层结构：

1. **表达DNA** — 句式偏好、词汇特征、节奏感、幽默方式
2. **心智模型** (3-7个) — 经三重验证（跨域复现、生成力、排他性）的认知框架
3. **决策启发式** (5-10条) — 「如果X则Y」的快速判断规则
4. **价值观与反模式** — 核心价值排序、反对的行为/思维、内在矛盾
5. **诚实边界** — 明确标注的局限性

## 工作流程

五阶段流水线 + 双 Agent 精炼：
- **Phase 0**: 需求澄清（直接路径/诊断路径）
- **Phase 1**: 6 Agent 并行多源采集（著作/对话/表达/他者/决策/时间线）
- **Phase 2**: 框架提炼 + 三重验证筛选
- **Phase 3**: Skill 构建（标准模板 + Agentic 回答工作流）
- **Phase 4**: 质量验证（已知/边缘/风格三项测试）
- **Phase 5**: 双 Agent 精炼（optimizer + creator 交叉评审）

**关键创新**: Agentic 回答工作流——遇到需要事实的问题先联网研究再发言，从「鹦鹉学舌」升级为「可靠思维顾问」。

## 预置 Skills

13 个人物 + 1 个主题，包含 Paul Graham、Elon Musk、Steve Jobs、Charlie Munger、Naval Ravikant、Andrej Karpathy 等。

## 与本 Wiki 的关联

- 属于 [[ai-skills|AI Skills 生态]] 的代表性项目——思维蒸馏方向
- 六路 Agent 并行采集是 [[ai-agent|AI Agent]] Multi-Agent 模式的实践
- 与 [[llm-wiki]] 知识管理模式互补：LLM Wiki 沉淀知识，nuwa-skill 蒸馏思维
- 与 [[rag|RAG]] 形成对比：RAG 是检索增强，nuwa-skill 是思维模式增强

## 来源

- [[../../raw/morning-briefs/2026-04-15.md|晨报 2026-04-15]]
- [[../../raw/morning-briefs/2026-04-20.md|晨报 2026-04-20]]
- [[../../../raw/articles/nuwa-skill-github.md|nuwa-skill GitHub 仓库]]
- [[sources/articles/nuwa-skill-github|源摘要：nuwa-skill]]
