---
title: "career-ops"
type: entity
subtype: product
tags: [tool, ai-skill, claude-code, github-trending]
created: 2026-04-15
updated: 2026-04-20
sources:
  - ../../raw/morning-briefs/2026-04-15.md
  - ../../raw/morning-briefs/2026-04-20.md
---

# career-ops

> 基于 Claude Code 的 AI 求职系统，14 种技能模式，Go dashboard，PDF 生成，批量处理，37K Stars

## 基本信息

- **GitHub**: santifer/career-ops
- **Stars**: 37,000+ (2026-04-20)，较早期的 33,861 增长
- **定位**: AI 求职助理
- **功能**:
  - 14 种 skill modes
  - Go dashboard
  - PDF 生成
  - 批量处理
- **基于**: Claude Code

## 与本 Wiki 的关联

- 基于 [[multi-agent]] 概念的垂直应用
- 代表 AI Agent 在 HR/求职领域的落地
- 属于 Claude Code 技能生态

## 来源

- [[../../raw/morning-briefs/2026-04-15.md|晨报 2026-04-15]]
- [[../../raw/morning-briefs/2026-04-20.md|晨报 2026-04-20]]
