...
**关联**：[[concepts/programming/monolith-async-worker|Monolith with Async Worker]] · [[concepts/microservices|微服务架构]]
...
**关联**：[[concepts/backend-architecture|后端架构]]
...
**关联**：[[concepts/programming/message-queue|消息队列]]
...
**关联**：[[concepts/programming/read-after-write|写后读问题]]
...
**关联**：[[concepts/microservices|微服务架构]]（Saga 章节）
...
**关联**：[[concepts/cloud-deployment|云服务部署]] · [[concepts/programming/canary-deployment|金丝雀部署]]
...
**关联**：[[concepts/cloud-deployment|云服务部署]]（可观测性章节）
...
后端栈和 [[synthesis/react-architecture-stack|React 架构栈]] 在思维上高度同构——都是**按维度分层 + 每层独立选工具**的分解。
...
**共同底层**：[[concepts/solid-principles|SOLID]]（单一职责、依赖反转）、[[concepts/clean-code|DRY/KISS/YAGNI]]、[[concepts/refactoring|重构]]。
...
- **AI/Agent 服务的后端栈长什么样？** LLM 调用天然耗时（秒～分钟级）+ 高失败率，L2 异步 + L3 幂等成为**默认配置而非可选**。[[concepts/programming/message-queue|消息队列]]里的 kaigao 项目实践就是这类范式的实例。
- **Database per Service 真的是微服务铁律吗？** 严格执行会让 L3 一致性代价飙升（跨库查询、跨库事务都要走 Saga）。业界开始出现"共享数据库但强边界"的折衷（每个服务只读写自己 schema）。

## 相关页面

- **L0 拓扑**：[[concepts/programming/monolith-async-worker|Monolith with Async Worker]] · [[concepts/microservices|微服务架构]]
- **L1 内部**：[[concepts/backend-architecture|后端架构]]（Layered / Hexagonal / Clean / CQRS）
- **L2 数据/异步**：[[concepts/programming/message-queue|消息队列]]（BullMQ / Outbox / 幂等 / SKIP LOCKED）
- **L3 一致性**：[[concepts/programming/read-after-write|写后读问题]]
- **L4 部署**：[[concepts/cloud-deployment|云服务部署]] · [[concepts/programming/canary-deployment|金丝雀部署]]
- **跨层业务参考**：[[concepts/programming/unified-payment-route|统一支付路由设计]]（L1 内部结构 + L2 异步 + L3 一致性综合案例）
- **前端对照**：[[synthesis/react-architecture-stack|React 架构栈]]

## 来源

- [[sources/articles/backend-architecture-article|后端架构知识资料]] — 架构模式 / 微服务 / 云部署的基础综述
